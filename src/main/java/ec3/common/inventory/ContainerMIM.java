...
public class ContainerMIM extends Container {
	...
	public ContainerMIM(InventoryPlayer par1InventoryPlayer, TileEntity par2) {
		inv = (IInventory)par2;
		addSlotToContainer(new SlotBoundEssence(inv, 0, -17, -17));
		
		for(int i = 0; i < 54; ++i) {
			addSlotToContainer(new SlotGeneric(inv, 1+i, 8+(i%9)*18, 8 + (i/9)*18));
		}
		...
		for(i = 0; i < 3; ++i) {
			for(int j = 0; j < 9; ++j) {
				addSlotToContainer(new Slot(par1InventoryPlayer, j + i*9 + 9, 8 + j*18, 126 + i*18));
			}
		}
		
		for(i = 0; i < 9; ++i) {
			addSlotToContainer(new Slot(par1InventoryPlayer, i, 8 + i*18, 188));
		}
	}
