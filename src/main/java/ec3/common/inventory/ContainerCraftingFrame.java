...
public class ContainerCraftingFrame extends Container {
		...
		public boolean isItemValid(ItemStack stk) {
			if(stk != null) {
				if(slotNumber != 9) {
					...
					Slot.class.cast(parent.inventorySlots.get(9)).onSlotChanged();
				}
				else {
					...
					if(!getHasStack()) {
						...
						if(rec != null) {
							for(int i = 0; i < 9; ++i) {
								parent.inventory.setInventorySlotContents(i, null);
								if(rec instanceof ShapelessRecipes) {
									ShapelessRecipes srec = (ShapelessRecipes) rec;
									if(srec.recipeItems.size() > i)
										parent.inventory.setInventorySlotContents(i, (ItemStack) srec.recipeItems.get(i));
								}
								if(rec instanceof ShapedRecipes) {
									ShapedRecipes srec = (ShapedRecipes) rec;
									if(srec.recipeItems.length > i)
										parent.inventory.setInventorySlotContents(i, (ItemStack) srec.recipeItems[i]);
								}
								if(rec instanceof ShapelessOreRecipe) {
									...
									if(srec.getInput().size() > i) {
										if(srec.getInput().get(i) != null) {
											...
											parent.inventory.setInventorySlotContents(i, (ItemStack)ust.possibleStacks.get(player.worldObj.rand.nextInt(ust.possibleStacks.size())));
										}
										else
											parent.inventory.setInventorySlotContents(i, null);
									}
								...
								Slot.class.cast(parent.inventorySlots.get(i)).onSlotChanged();
							}
						}
					}
					
					inventory.setInventorySlotContents(slotNumber, settedRec == null || settedRec.getRecipeOutput() == null ? null : settedRec.getRecipeOutput().copy());
				}
			}
			return false;
		}
	...
	public ContainerCraftingFrame(EntityPlayer p, InventoryCraftingFrame inv) {
		...
		for(int o = 0; o < 9; ++o) {
			addSlotToContainer(new SlotFake(p, this, inv, o, 30+(o/3*18), 17+(o%3*18)));
		}
		
		addSlotToContainer(new SlotFake(p, this, inv, 9, 124, 17+18));
