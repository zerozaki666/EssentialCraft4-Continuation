	...
	public ItemStack transferStackInSlot(EntityPlayer p_82846_1_, int p_82846_2_) {
		...
		if(slot != null && slot.getHasStack()) {
			...
			if(p_82846_2_ < inventory.getSizeInventory()) {
				if(!mergeItemStack(itemstack1, inventory.getSizeInventory(), 36+inventory.getSizeInventory(), true)) {
			...
			if(p_82846_2_ >= inventory.getSizeInventory() && p_82846_2_ < 27+inventory.getSizeInventory()) {
				if(!mergeItemStack(itemstack1, 27+inventory.getSizeInventory(), 36+inventory.getSizeInventory(), false)) {
					if(itemstack1.stackSize == 0)
						slot.putStack((ItemStack)null);
					return null;
				}
			}
			else if(p_82846_2_ >= 27+inventory.getSizeInventory() && p_82846_2_ < 36+inventory.getSizeInventory() && !mergeItemStack(itemstack1, inventory.getSizeInventory(), 27+inventory.getSizeInventory(), false)) {
				if(itemstack1.stackSize == 0)
					slot.putStack((ItemStack)null);
				return null;
			}
