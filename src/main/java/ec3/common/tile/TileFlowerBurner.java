		...
		if(!worldObj.isBlockIndirectlyGettingPowered(xCoord, yCoord, zCoord)) {
			if(!worldObj.isRemote) {
				if(burnedFlower == null && getMRU() < getMaxMRU()) {
					int offsetX = (int)(MathUtils.randomDouble(worldObj.rand)*8);
					int offsetZ = (int)(MathUtils.randomDouble(worldObj.rand)*8);
					...
					if(burnedFlower != null) {
						...
						if(burnTime <= 0) {
							if(createDeadBush)
								worldObj.setBlock((int)burnedFlower.x, (int)burnedFlower.y, (int)burnedFlower.z, Blocks.deadbush, 0, 3);
							else
								worldObj.setBlock((int)burnedFlower.x, (int)burnedFlower.y, (int)burnedFlower.z, Blocks.air, 0, 3);
							burnedFlower = null;
						}
					}
				}
			...
			if(burnedFlower != null && burnTime > 0) {
				worldObj.spawnParticle("flame", burnedFlower.x+0.5F + MathUtils.randomFloat(worldObj.rand)*0.3F, burnedFlower.y+0.1F + worldObj.rand.nextFloat()/2, burnedFlower.z+0.5F + MathUtils.randomFloat(worldObj.rand)*0.3F, 0, 0, 0);
				--burnTime;
				if(burnTime <= 0) {
					for(int t = 0; t < 200; ++t)
